...
// CRC-16 (CCITT/XModem: poly 0x1021, init 0xFFFF, no final XOR), returned as 2 little-endian
// bytes. Byte-exact port of PumpX2Kit `Bytes.calculateCRC16`
// (upstream com.jwoglom.pumpx2.pump.messages.helpers.Bytes).
//
// The Swift/Java reference accumulates in a 32-bit int WITHOUT masking to 16 bits between
// iterations (it relies on `(i << 8)` truncating to 32 bits and an unsigned `i >> 8`). Here we
// use the mathematically-equivalent canonical 16-bit form, keeping `crc` in 0..0xFFFF each step.
...
module Crc16 {
    ...
    // Returns the CRC-16 of `bytes` as a 2-byte little-endian ByteArray [lo, hi].
    function calculate(bytes as Lang.ByteArray) as Lang.ByteArray {
        var crc = 0xFFFF;
        var n = bytes.size();
        for (var k = 0; k < n; k++) {
            var idx = ((crc >> 8) ^ (bytes[k] & 0xFF)) & 0xFF;
            crc = ((crc << 8) & 0xFFFF) ^ TABLE[idx];
        }
