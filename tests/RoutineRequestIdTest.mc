...
// Pins the ROUTINE request-id mint (statusRead / statusReadFresh / dismissAlert — every
// hot-path call site EXCEPT the bolus/resume dose-authorizing send). RemoteComm.newRequestId()
// (see RequestIdTest.mc) does a Storage.get+set on EVERY mint; the routine callers fire every ~15s
// (pollTick), on every View.onShow, and on every wearer dismiss-confirm — turning that durable
